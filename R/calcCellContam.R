...
#' Title Given a single-cell expression matrix, summarize marker expression as log sum across multiple cell types
#'
#' @param expr_matrix single cell expression matrix (samples by genes, with gene names as column names)
#' @param markerList list of lists defining markers for various cell types
#' @param colwise boolean for whether matrix is stored as samples by genes (T) or genes by cells (F)
#'
#' @return Dataframe with cells by cell types, which each cell type (as a column) showing summed expression of cell type markers
#' @export
#'
#' @examples
