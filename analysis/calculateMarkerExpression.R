...
# compare_cell_types = setdiff(names(markers), c('Ndnf', 'Pvalb', 'Sncg')) %>% make.names()
...
zeisel_contam_all_broad = calcContamAllTypes(zeisel_expr_norm, markers)
...
zeisel_contam_all_sub = calcContamAllTypes(zeisel_expr_norm, markers)
...
for (i in 1:length(patch_seq_datasets)){
  ...
  marker_sums = lapply(dataset_cell_types, function(cell_type){
    ...
    dissoc_corr = rbind(compare_expr_profile %>% t() %>% as.data.frame, df[, markers[c(curr_marker_type, compare_cell_types)] %>% unlist %>% unique] %>% log2) %>% t() %>% cor(method = 'spearman')
  ...
  marker_sum_df = bind_cols(dataset$joined_df %>% select(one_of('sample_id')), marker_sums)
  ...
  patch_seq_datasets[[i]]$contam_scores = marker_sum_df# %>% select(one_of(c('sample_id', names(markers))))
  rownames(patch_seq_datasets[[i]]$contam_scores) = dataset$joined_df$sample_id
  patch_seq_datasets[[i]]$contam_scores = marker_sum_df# %>% select(one_of(c('sample_id', names(markers))))
  patch_seq_datasets[[i]]$contam_scores = merge(patch_seq_datasets[[i]]$contam_scores, dataset$joined_df[, c('sample_id', 'major_type')])
  #
...
calculatePatchSeqQCMetrics = function(patch_seq_df, comparison_dataset = 'aibs', markers){
  ...
  dataset_cell_types = levels(patch_seq_df$major_type)
  marker_sums = lapply(dataset_cell_types, function(cell_type){

    curr_marker_type = patch_seq_df$major_type[patch_seq_df$major_type== cell_type, 'contam_type'][1] %>% as.character
    curr_marker_type = paste0(curr_marker_type, '_on')
    curr_marker_list = markers[[curr_marker_type]]
